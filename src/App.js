...
import { loadData, saveEntries, saveExpenses, savePayments } from './drive';
import './App.css';
...
// Parse Hatsun payment SMS
// "Milk poured amount Rs.1697.20  for 01.05.2026 to 10.05.2026 is deposited in your account..."
function parseHatsunPaymentSMS(txt) {
  const result = {};
  const amtMatch = txt.match(/Rs\.?([\d,]+\.?\d*)/i);
  if (amtMatch) result.amount = parseFloat(amtMatch[1].replace(',', ''));

  const dateRangeMatch = txt.match(/for\s+(\d{2})\.(\d{2})\.(\d{4})\s+to\s+(\d{2})\.(\d{2})\.(\d{4})/i);
  if (dateRangeMatch) {
    result.fromDate = `${dateRangeMatch[3]}-${dateRangeMatch[2]}-${dateRangeMatch[1]}`;
    result.toDate = `${dateRangeMatch[6]}-${dateRangeMatch[5]}-${dateRangeMatch[4]}`;
  }
  // Determine cycle: which payment day (3rd, 13th, 23rd)
  if (result.toDate) {
    const day = parseInt(result.toDate.split('-')[2]);
    if (day <= 10) result.cycle = '1st–10th';
    else if (day <= 20) result.cycle = '11th–20th';
    else result.cycle = '21st–EOM';
  }
  return result;
}
    ...
    <div className="calendar-wrap">
      ...
      <div className="cal-grid">
        {cells.map((d, i) => {
          ...
          // payment days
          const isPayDay = d === 3 || d === 13 || d === 23;
          return (
            <div key={d} className={`cal-cell${data ? ' has-data' : ''}${isToday ? ' is-today' : ''}${isPayDay ? ' is-payday' : ''}`}>
              <div className="cal-date-num">{d}{isPayDay && <span className="payday-dot" title="Payment day">₹</span>}</div>
              {data && (
                <div className="cal-data">
                  <div className="cal-qty">{data.qty.toFixed(1)}L</div>
                  <div className="cal-rate">₹{avgRate.toFixed(0)}</div>
...
export default function App() {
  ...
  const [parsedPaymentPreview, setParsedPaymentPreview] = useState(null);
  ...
  const [paymentFilterMonth, setPaymentFilterMonth] = useState(thisMonth());
  ...
  const syncPayments = useCallback(async (p) => {
    if (!token || !folderId) return;
    setSyncing(true);
    try { await savePayments(token, folderId, p); } catch { toast('Sync failed'); }
    setSyncing(false);
  }, [token, folderId]);
  ...
  // Hatsun payment SMS
  const handleParsePaymentSMS = () => {
    if (!paymentSmsText.trim()) { toast('Paste the Hatsun SMS first'); return; }
    const parsed = parseHatsunPaymentSMS(paymentSmsText);
    if (!parsed.amount) { toast('Could not read payment SMS format'); return; }
    setParsedPaymentPreview(parsed);
  };

  const confirmPayment = async () => {
    if (!parsedPaymentPreview) return;
    const p = { ...parsedPaymentPreview, id: Date.now(), receivedDate: today() };
    const updated = [p, ...payments];
    setPayments(updated); await syncPayments(updated);
    toast('Payment recorded!');
    setPaymentSmsText(''); setParsedPaymentPreview(null);
  };

  const deletePayment = async (id) => {
    const updated = payments.filter(p => p.id !== id);
    setPayments(updated); await syncPayments(updated); toast('Deleted');
  };
  ...
  const paymentSummary = () => {
    const allP = payments.filter(p => p.fromDate?.startsWith(paymentFilterMonth) || p.receivedDate?.startsWith(paymentFilterMonth));
    return allP.reduce((s,p) => s + p.amount, 0);
  };
  ...
  const filteredPayments = payments
    .filter(p => !paymentFilterMonth || p.fromDate?.startsWith(paymentFilterMonth) || p.receivedDate?.startsWith(paymentFilterMonth))
    .sort((a,b) => (b.receivedDate||'').localeCompare(a.receivedDate||''));

  const summaryStats = getSummaryStats(summaryMode);

  const NAV_TABS = ['dashboard','log','calendar','summary','payments','expenses','profit'];
    ...
    <div className="app">
      ...
      <div className="content">
        ...
        {page === 'dashboard' && (() => {
          ...
          return (
            <div>
              ...
              {recentPayment && (
                <div>
                  <div className="section-title syne">Last Hatsun Payment</div>
                  <div className="payment-banner">
                    <div className="payment-banner-left">
                      <div className="payment-banner-label">Amount Credited</div>
                      <div className="payment-banner-amt syne">Rs. {recentPayment.amount?.toFixed(2)}</div>
                      <div className="payment-banner-range">{fmtDate(recentPayment.fromDate)} → {fmtDate(recentPayment.toDate)}</div>
                    </div>
                    <div className="payment-banner-cycle">{recentPayment.cycle}</div>
                  </div>
                </div>
              )}
        ...
        {page === 'calendar' && (
          <div>
            ...
            <div className="cal-legend">
              <span className="cal-legend-item"><span className="cal-legend-dot payday-col">₹</span> Payment day (3rd, 13th, 23rd)</span>
        ...
        {page === 'payments' && (
          <div>
            {/* Paste Hatsun SMS */}
            <div className="card payment-sms-card">
              <div className="card-title syne">🏦 Hatsun Payment SMS</div>
              <p className="payment-hint">Paste the SMS received on 3rd, 13th or 23rd of month</p>
              <textarea className="sms-area" value={paymentSmsText} onChange={e=>setPaymentSmsText(e.target.value)}
                placeholder={'Milk poured amount Rs.1697.20  for 01.05.2026 to 10.05.2026 is deposited in your account...'} />
              <div className="row-gap">
                <button className="btn btn-primary" onClick={handleParsePaymentSMS}>Auto Detect</button>
                <button className="btn btn-ghost" onClick={()=>{setPaymentSmsText('');setParsedPaymentPreview(null);}}>Clear</button>
              </div>
              {parsedPaymentPreview && (
                <div className="preview-box">
                  <div className="preview-title syne">✅ Payment Detected</div>
                  <div className="preview-row"><span>Amount</span><b className="amt">Rs. {parsedPaymentPreview.amount?.toFixed(2)}</b></div>
                  <div className="preview-row"><span>Period</span><b>{fmtDate(parsedPaymentPreview.fromDate)} → {fmtDate(parsedPaymentPreview.toDate)}</b></div>
                  <div className="preview-row"><span>Cycle</span><b>{parsedPaymentPreview.cycle}</b></div>
                  <div className="row-gap" style={{marginTop:10}}>
                    <button className="btn btn-primary" onClick={confirmPayment}>✅ Record Payment</button>
                    <button className="btn btn-ghost" onClick={()=>setParsedPaymentPreview(null)}>Cancel</button>
                  </div>
                </div>
              )}
            </div>
            ...
            <div className="table-wrap">
              <div className="table-header">
                <span className="syne" style={{fontWeight:700}}>Payment History</span>
                <input type="month" value={paymentFilterMonth} onChange={e=>setPaymentFilterMonth(e.target.value)} />
              </div>
              {filteredPayments.length > 0 && (
                <div className="exp-total-bar">
                  <span>Total ({filteredPayments.length} payments)</span>
                  <span className="exp-total-amt" style={{color:'var(--green-mid)'}}>Rs. {paymentSummary().toFixed(2)}</span>
                </div>
              )}
              {filteredPayments.map(p => (
                <div className="expense-item" key={p.id}>
                  <div>
                    <div className="exp-name">
                      <span className="payment-cycle-badge">{p.cycle}</span>
                      {' '}{fmtDate(p.fromDate)} → {fmtDate(p.toDate)}
                    </div>
                    <div className="exp-date">Received: {fmtDate(p.receivedDate)}</div>
                  </div>
                  <div className="row-gap">
                    <div className="exp-amt syne" style={{color:'var(--green-mid)'}}>+Rs.{p.amount?.toFixed(2)}</div>
                    <button className="btn-del" onClick={()=>deletePayment(p.id)}>✕</button>
                  </div>
                </div>
              ))}
              {!filteredPayments.length && <div className="empty">No payments recorded</div>}
            </div>

            {/* Cycle tracker */}
            <div className="card">
              <div className="card-title syne">📅 Payment Cycle Tracker</div>
              <div className="cycle-tracker">
                {[
                  { label: '1st–10th', day: '3rd', cycle: '1st–10th' },
                  { label: '11th–20th', day: '13th', cycle: '11th–20th' },
                  { label: '21st–EOM', day: '23rd', cycle: '21st–EOM' },
                ].map(c => {
                  const found = payments.find(p => p.cycle === c.cycle && p.receivedDate?.startsWith(thisMonth()));
                  return (
                    <div key={c.cycle} className={`cycle-item${found ? ' received' : ''}`}>
                      <div className="cycle-period">{c.label}</div>
                      <div className="cycle-day">Paid on {c.day}</div>
                      {found
                        ? <div className="cycle-status received-text">✅ Rs.{found.amount?.toFixed(2)}</div>
                        : <div className="cycle-status pending-text">⏳ Pending</div>
                      }
                    </div>
                  );
                })}
