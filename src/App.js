...
export default function App() {
  ...
  const [bonusGiven, setBonusGiven] = useState(() => {
    const b = localStorage.getItem('hap_bonus');
    return b ? JSON.parse(b) : {};
  });
  const [folderId, setFolderId] = useState(null);
  const [page, setPage] = useState('dashboard');
  const [overviewMode, setOverviewMode] = useState('today');
  ...
  const logout = () => {
    ...
    setToken(null); setUser(null); setEntries([]); setExpenses([]);
  };
  ...
  // BONUS 0.5L @ Rs.25
  const toggleBonus = (date) => {
    const updated = { ...bonusGiven, [date]: !bonusGiven[date] };
    setBonusGiven(updated);
    localStorage.setItem('hap_bonus', JSON.stringify(updated));
    toast(updated[date] ? '✅ 0.5L bonus marked given! +Rs.25' : 'Bonus unmarked');
  };

  const bonusIncomeForMonth = (month) => {
    return Object.entries(bonusGiven)
      .filter(([date, given]) => given && date.startsWith(month))
      .length * 25;
  };
  ...
  const profitData = (m) => {
    ...
    const bonus = bonusIncomeForMonth(m);
    return { income, exp, bonus, profit: income + bonus - exp, litres: me.reduce((s,e)=>s+e.qty,0) };
  };
  ...
  const getStats = (filtered) => {
    ...
    return {
      ...
      avgFat: avg(filtered.filter(e=>e.fat).map(e=>e.fat)),
      avgSnf: avg(filtered.filter(e=>e.snf).map(e=>e.snf)),
      avgRate: avg(filtered.filter(e=>e.rate).map(e=>e.rate)),
  ...
  const weekE = entries.filter(e => weekDates.includes(e.date));
  const monthE = entries.filter(e => e.date?.startsWith(thisMonth()));

  const overviewEntries = overviewMode === 'today' ? todayE : overviewMode === 'week' ? weekE : monthE;
  const stats = getStats(overviewEntries);
  ...
  const todayBonus = bonusGiven[today()];
      ...
      <div className="content">
          ...
          <div>
            ...
            <div className="overview-tabs">
              {['today','week','month'].map(m => (
                <button key={m} className={`ov-tab${overviewMode===m?' active':''}`} onClick={()=>setOverviewMode(m)}>
            ...
            {/* BONUS BUTTON */}
            <div className={`bonus-bar ${todayBonus?'given':''}`} onClick={()=>toggleBonus(today())}>
              <div>
                <div className="bonus-title">🥛 Morning 0.5L Bonus</div>
                <div className="bonus-sub">Daily 0.5 L @ Rs.25 to neighbour</div>
              </div>
              <div className={`bonus-btn ${todayBonus?'given':''}`}>
                {todayBonus ? '✅ Given' : 'Mark Given'}
              </div>
            </div>
            ...
            {stats ? (
              <div className="summary-grid">
                <div className="sum-card accent">
                  ...
                  <div className="sum-value">{stats.qty.toFixed(2)} L</div>
                  <div className="sum-sub">{stats.count} session(s)</div>
                </div>
                <div className="sum-card accent">
                  <div className="sum-label">Total Amount</div>
                  <div className="sum-value">Rs.{stats.amount.toFixed(2)}</div>
                  <div className="sum-sub">Income</div>
                </div>
                <div className="sum-card">
                  <div className="sum-label">Avg Rate</div>
                  <div className="sum-value">Rs.{stats.avgRate.toFixed(2)}</div>
                  <div className="sum-sub">Per Litre</div>
                </div>
                <div className="sum-card">
                  <div className="sum-label">Avg Fat %</div>
                  <div className="sum-value">{stats.avgFat.toFixed(2)}%</div>
                  <div className="sum-sub">Avg SNF: {stats.avgSnf.toFixed(2)}%</div>
                </div>
              </div>
            ) : (
              <div className="empty-overview">No entries for {overviewMode === 'today' ? 'today' : overviewMode === 'week' ? 'this week' : 'this month'}</div>
            )}
        ...
        {page === 'log' && (
          <div>
            <div className="card sms-card">
              ...
              {parsedPreview && (
                <div className="preview-box">
                  ...
                  <div className="preview-row"><span>Date</span><b>{parsedPreview.date} / {parsedPreview.session}</b></div>
            ...
            <div className="card">
              <div className="card-title syne">Manual Entry</div>
              <div className="form-grid">
                ...
                <div className="form-group"><label>Rate (Rs/Lt)</label><input type="number" value={fRate} onChange={e=>setFRate(e.target.value)} placeholder="0.00" /></div>
                ...
                <div className="form-group full"><label>Amount (Auto)</label><div className="amt-display syne">Rs. {calcAmt()}</div></div>
              </div>
        ...
        {page === 'profit' && (
          <div>
            ...
            {(() => {
              const { income, exp, bonus, profit, litres } = profitData(profitMonth);
              return (
                <div className="profit-cards">
                  <div className="profit-card"><div><div className="pc-label">Milk Income</div><div className="pc-value green">Rs. {income.toFixed(2)}</div></div><div className="pc-icon">🥛</div></div>
                  <div className="profit-card"><div><div className="pc-label">Bonus Income</div><div className="pc-value green">Rs. {bonus.toFixed(2)}</div></div><div className="pc-icon">🤝</div></div>
            ...
            <div className="table-wrap">
              ...
              <table>
                <thead><tr><th>Month</th><th>Income</th><th>Bonus</th><th>Expenses</th><th>Profit</th></tr></thead>
                <tbody>
                  {getAllMonths().map(m => {
                    ...
                    return (
                      <tr key={m} style={{background: m===profitMonth?'var(--green-pale)':''}}>
                        ...
                        <td className="amt">Rs.{d.income.toFixed(2)}</td>
                        <td className="amt">Rs.{d.bonus.toFixed(2)}</td>
